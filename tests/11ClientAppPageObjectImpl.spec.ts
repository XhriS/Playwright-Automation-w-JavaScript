...
import {customtest} from '../utils_TypeScript/test-base'; //Passing test data as ficture by extending test annotation behavior
import {PageObjectManager} from '../pageobjects_TypeScript/PageObjectManager';
...
test(`@Web Client App Login for ${data.productName}`,async ({page})=> //Annonymous function. You use curly braces to define parameters as fixtures for Playwright to recognize them as global variables
{
    ...
    countryName = "India";
    let expectedConfText: string;
    expectedConfText = " Thankyou for the order. ";
    ...
    let orderId: any;
    orderId = await orderConfirmationPage.verifyConfirmationMessageAndOrderID(expectedConfText);    
    console.log(orderId);
